...
var (
	...
	flHdNmSz = [...]nmSzPair{
		{"Machine",         2}, {"NumberOfSections",     2}, {"TimeDataStamp",   4}, {"PointerToSymbolTable", 4}, 
	...
	flHdCharacters = map[uint8]string{
	...
	scnHdCharacters = map[uint8]string{
...
func getCharacteristicsVal(val any) string {
	...
	for _, v := range floatChars {
		builder.WriteString((*ptrChars)[uint8(math.Log2(v))])
		builder.WriteRune('\n')
	}
...
func ParsePE(wholeFile []byte, sys int8) *PeHeader {
	...
	pe.optionalHeader = parseHeader(offset, szOfOpHd, nmSzSlice, func(i int, d uint) string {
		switch i {
		case subsysIdx: return subsysNumVal[uint16(d)]
		case 0:
			switch d {
